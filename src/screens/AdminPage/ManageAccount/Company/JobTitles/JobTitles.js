...
const JobTitles = ({navigation}) => {
    const {profile} = useAuthContext()

    const {data:jobtitles,loading,error} = useFetchCollection('jobtitles',where('companyId','==',profile.companyId),orderBy('name'))
    ...
    const searchList=()=>{
     const filteredtitle= jobtitles?.filter(title =>
        title.name.toLowerCase().includes(searchText.toLowerCase())
      );
     
      setNoMatch(filteredtitle.length === 0)
    ...
    const renderItem=({item})=>(
      <BranchCard item={item} onPress={()=>navigation.navigate('jobtitleform',{data:item,action:'edit'})}/>
    )
    ...
    return (
      <View className='flex-1 p-4 bg-zinc-100 relative'>
        ...
        :
        <FlatList
        data={filtered.length ===0 ? jobtitles:filtered}
        renderItem={renderItem}
        keyExtractor={(item) => item.id}
        showsVerticalScrollIndicator={false}
  />}
  <TouchableOpacity className='absolute right-4 bottom-10' onPress={()=>navigation.navigate('jobtitleform')}>
